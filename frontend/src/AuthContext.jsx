...
export function AuthProvider({ children }) {
    ...
    const register = async (name, email, password, phone, role, adminKey, dob, gender, officeDetails) => {
        const res = await fetch(`${API_BASE}/api/auth/register`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ name, email, password, phone, role, adminKey, dob, gender, officeDetails }),
        });
        const data = await res.json();
        if (!res.ok) throw new Error(data.error);
        sessionStorage.setItem('token', data.token);
    ...
    return (
        <AuthContext.Provider value={{ user, setUser, token, login, register, logout, loading, sendOtp, verifyOtp, resetPassword }}>
            {children}
